...
void TagMining::BuildTagListMap() {
  ifstream train_parsed_file(working_dir + "\\train_parsed.txt", ios_base::in);
  string ln_buffer, word_buffer;
  ...
  while (getline(train_parsed_file, ln_buffer)) {
    item_no = stoi(ln_buffer);
    if (item_no % 100 == 0)
      cout << item_no << "-th item is processed.\n";
    ...
    stringstream ln_sstream(ln_buffer);
    while (ln_sstream >> word_buffer)
      if (correlation_map.count(word_buffer)) {
        if (word_list.count(word_buffer))
          word_list[word_buffer] += 1;
        else
          word_list.insert(make_pair(word_buffer, 1));
    ...
    tag_list_map.insert(make_pair(item_no, tag_list));
  }
...
void TagMining::Train() {
  ...
  while (getline(train_parsed_file, ln_buffer)) {
    ...
    Bounds init_bounds = {0, 1000000, 0, 1000000};
    for (map< string, set<int> >::iterator map_it = inverted_tag_map.begin();
         map_it != inverted_tag_map.end(); ++map_it)
      cor_thresholds.insert(make_pair(map_it->first, init_bounds));

    for (map<string, double>::iterator map_it = tag_list.begin(); map_it != tag_list.end(); ++map_it) {
      if (local_tags.count(map_it->first)) {
        if (map_it->second > cor_thresholds[map_it->first].tag_cor_bounds[0])
          cor_thresholds[map_it->first].tag_cor_bounds[0] = map_it->second;
        if (map_it->second < cor_thresholds[map_it->first].tag_cor_bounds[1])
          cor_thresholds[map_it->first].tag_cor_bounds[1] = map_it->second;
      }
      else {
        if (map_it->second > cor_thresholds[map_it->first].tag_cor_bounds[2])
          cor_thresholds[map_it->first].tag_cor_bounds[2] = map_it->second;
        if (map_it->second < cor_thresholds[map_it->first].tag_cor_bounds[3])
          cor_thresholds[map_it->first].tag_cor_bounds[3] = map_it->second;
      }
    }
  }
  ...
  ofstream tag_cor_bounds_file(working_dir + "\\tag_cor_bounds.txt", ios_base::out);

  int bad_count = 0, worse_count = 0;
  for (map<string, Bounds>::iterator map_it = cor_thresholds.begin();
       map_it != cor_thresholds.end(); ++map_it) {
    if ((map_it->second).tag_cor_bounds[1] < (map_it->second).tag_cor_bounds[2]) {
      ++bad_count;
      if ((map_it->second).tag_cor_bounds[0] < (map_it->second).tag_cor_bounds[2])
        ++worse_count;
    }
    tag_cor_bounds_file << map_it->first << " " << (map_it->second).tag_cor_bounds[0] << " "
    << (map_it->second).tag_cor_bounds[1] << " " << (map_it->second).tag_cor_bounds[2] << " "
    << (map_it->second).tag_cor_bounds[3] << endl;
  }

  cout << "bad: " << bad_count << ",  worse: " << worse_count << endl;

  tag_cor_bounds_file.close();
}
